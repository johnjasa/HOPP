...
class ElectrolyzerCluster(object):
    ...
    def __init__(
        ...
        uptime_hours_until_eol=None,
        ramp_rate_percent=None,
        turndown_ratio=None,
        cold_start_delay=None,
        anode_pressure_bar=None,
        cathode_pressure_bar=None,
        ...
        """
        self.electrolyzer_type = electrolyzer_type

        if uptime_hours_until_eol is None:
            if self.electrolyzer_type == "ALK":
                uptime_hours_until_eol = 80000
            else:
                uptime_hours_until_eol = 77600

        if ramp_rate_percent is None:
            if self.electrolyzer_type == "ALK":
                ramp_rate_percent = 0.2
            else:
                ramp_rate_percent = 0.99

        if turndown_ratio is None:
            if self.electrolyzer_type == "ALK":
                turndown_ratio = 0.25
            else:
                turndown_ratio = 0.10

        if cold_start_delay is None:
            if self.electrolyzer_type == "ALK":
        ...
        self.ramp_rate_percent = (
            ramp_rate_percent  # percent of rated current per second
        )
        ...
        self.BOL_design_info.update(
            {"EOL Cell Voltage Degradation Value [V/cell]": self.d_eol}
        )
        # CELL DEGRADATION RATES
    ...
    def describe_degradation_rates(self):
        n_hours_until_eol_uptime = self.d_eol / (
            self.steady_deg_rate * self.V_cell_nominal * 3600
        )
        n_off_cycles_until_eol = self.d_eol / (self.onoff_deg_rate)
        self.BOL_design_info.update(
            {
                "Max Stack Operational Hours Until EOL": n_hours_until_eol_uptime,
    ...
    def system_design(self, T_stack):
        ...
        if self.electrolyzer_type == "PEM":
            ...
            cell_nominal_h2_kg = self.cell_H2_production_rate(
                self.nominal_current, T_stack
            )

        self.stack_nominal_h2_kg = cell_nominal_h2_kg * self.n_cells  # [kg H2/dt-stack]
        self.cluster_nominal_h2_kg = (
            self.stack_nominal_h2_kg * self.n_stacks
        )  # [kg H2/dt-cluster]

        if self.electrolyzer_type == "PEM":
            cell_min_h2_kg = self.cell_H2_production_rate(self.min_current)
        else:
            cell_min_h2_kg = self.cell_H2_production_rate(self.min_current, T_stack)
        ...
        self.current_ramp_rate = (
            self.ramp_rate_percent * self.nominal_current * self.dt
        )  # A/dt

        # TODO: add these values to BOL design info outputs
        key_desc = [
            "Cluster H2 Production [kg/dt]",
            "Cluster Power [kW]",
            "Stack Current [A]",
            "Cell Voltage [V/cell]",
            "Efficiency [kWh/kg]",
        ...
        vals = [
            ...
            self.V_cell_nominal,
            self.cluster_rating_kW / self.cluster_nominal_h2_kg,
        ]

        keys += ["BOL Minimum {}".format(k) for k in key_desc]
        vals += [
            self.cluster_min_h2_kg,
            self.min_cluster_power_kW,
            self.min_current,
            self.V_cell_min,
            self.min_cluster_power_kW / self.cluster_min_h2_kg,
        ]

        self.BOL_design_info.update(dict(zip(keys, vals)))
        self.BOL_design_info.update(
            {
                "n_stacks/cluster": self.n_stacks,
                "n_cells/stack": self.n_cells,
                "Minimum Stack Power [kW]": self.min_stack_power_kW,
            }
        )
        self.BOL_design_info.update({"Stack Operating Temperature [C]": self.T_stack})
        if self.electrolyzer_type == "PEM":
            self.BOL_design_info.update(
                {"Stack Anode Pressure [bar]": self.anode_pressure}
            )
            self.BOL_design_info.update(
                {"Stack Cathode Pressure [bar]": self.cathode_pressure}
            )
        else:
            self.BOL_design_info.update(
                {"Stack Anode Pressure [bar]": self.pressure_operating}
            )
            self.BOL_design_info.update(
                {"Stack Cathode Pressure [bar]": self.pressure_operating}
            )
    ...
    def initalize_outputs(self, plant_life: int, run_LTA: bool, debug_mode: bool):
        ...
        if run_LTA:
            ...
            lta_keys = [
                "Capacity Factor [-]",
                "Refurbishment Schedule [stacks replaced/year]",
            ...
            self.LTA_results_annual = pd.DataFrame(
                data=x, index=years, columns=lta_keys
            )
            self.LTA_results_average = dict(zip(lta_keys, [None] * len(lta_keys)))
            self.LTA_results_average.pop(
                "Refurbishment Schedule [stacks replaced/year]"
            )
        ...
        self.timeseries_results = dict(
            zip(timeseries_keys, [None] * len(timeseries_keys))
        )
    ...
    def find_eol_voltage_val(self, eol_eff_percent_loss):
        ...
        d_eol = (
            (self.nominal_current * self.V_cell_nominal) / i_eol
        ) - self.V_cell_nominal
        # eol_eff_kWh_per_kg = bol_eff_kWh_per_kg*(1+eol_eff_percent_loss/100)

        self.BOL_design_info.update(
            {"EOL Stack Rated H2 Production [kg/dt]": h2_eol_stack}
        )

        return d_eol

    def cluster_external_power_supply(self, input_external_power_kW):
        total_curtailed_power_kW = self.cluster_calc_curtailed_power(
    ...
    def check_current_bounds(self, I_stack_nom):
        ...
        I_stack_nom_sat = np.where(
            I_stack_nom_sat > self.nominal_current,
            self.nominal_current,
            I_stack_nom_sat,
        )
    ...
    def run_cluster(self, I_stack_nom):
        ...
        if self.electrolyzer_type == "PEM":
            hydrogen_produced_kg_nom = (
                self.cell_H2_production_rate(I_stack) * self.n_cells * self.n_stacks
            )
        else:
            hydrogen_produced_kg_nom = (
                self.cell_H2_production_rate(I_stack, self.T_stack)
                * self.n_cells
                * self.n_stacks
            )
        ...
        self.add_simulation_results(
            "Total Hydrogen Production [kg/sim]", np.sum(hydrogen_produced_kg), False
        )
        self.add_simulation_results(
        ...
        if self.run_LTA:
            ...
            avg_years_until_replacement = int(
                np.floor(time_between_replacement_hours / 8760)
            )
            self.LTA_results_average.update(
                {"Years between stack replacement": avg_years_until_replacement}
            )

        self.simulation_results.update(
            {"Simulation Time Until Replacement [hrs]": time_between_replacement_hours}
        )
        self.simulation_results.update(
            {"Simulation Stack Life [hrs]": stack_life_hours}
        )
        self.simulation_results.update(
            {
                "Simulation Capacity Factor [-]": np.sum(hydrogen_produced_kg)
                / (self.cluster_nominal_h2_kg * len(hydrogen_produced_kg))
            }
        )
    ...
    def estimate_stack_life(self, V_deg, cluster_status):
        ...
        stack_life = (
            (1 / frac_of_life_used) * operational_time_dt * (self.dt / 3600)
        )  # [hrs]
    ...
    def estimate_time_between_replacement(self, V_deg):
        ...
        time_between_replacement = (
            (1 / frac_of_life_used) * sim_time_dt * (self.dt / 3600)
        )
    ...
    def run_LTA_analysis(self, V_deg, V_cell_nom, I_stack_nom):
        if self.electrolyzer_type == "PEM":
            from greenheart.simulation.technologies.hydrogen.electrolysis.PEM_LTA import (
                PEM_LTA,
            )

            lta = PEM_LTA(self)
        else:
            from greenheart.simulation.technologies.hydrogen.electrolysis.alkaline_LTA import (
                alkaline_LTA,
            )
        ...
        if self.penalize_hydrogen_production:
            lta.annual_performance_for_degradation_applied_to_output(
                V_deg, V_cell_nom, I_stack_nom
            )
        else:
            lta.annual_performance_for_degradation_applied_to_input(
                V_deg, V_cell_nom, I_stack_nom
            )
    ...
    def run_cluster_variable_power(self, input_external_power_kW):
        ...
        self.add_simulation_results(
            "Total Curtailed Power [kW/sim]", np.sum(total_curtailed_power_kW), False
        )
        self.add_simulation_results(
            "Power Curtailed [kW]", total_curtailed_power_kW, True
        )

        # 0b. check cluster power input within load bounds
        input_external_power_kW = self.cluster_external_power_supply(
            input_external_power_kW
        )
        self.add_simulation_results(
            "Actual Power Input [kW]", input_external_power_kW, True
        )
        # 1. convert power to nominal (undegraded) current
        power_per_stack_kW = input_external_power_kW / self.n_stacks
        I_stack_nom = stack_power_to_current(
            (power_per_stack_kW, self.T_stack), *self.curve_coeff
        )
    ...
    def run_cluster_hydrogen_demand(self, H2_required_cluster_kg):
        I_stack_nom, input_external_power_kW = (
            self.calc_current_power_required_for_hydrogen_demand(H2_required_cluster_kg)
        )
        # 0b. check cluster power input within load bounds
        total_curtailed_power_kW = self.cluster_calc_curtailed_power(
            input_external_power_kW
    ...
    def calc_current_power_required_for_hydrogen_demand(self, H2_required_cluster_kg):
        ...
        power_reqd_kW = (
            I_reqd * (V_reqd + V_deg_est) * self.n_cells * self.n_stacks
        ) / 1e3
    ...
    def cluster_calc_curtailed_power(self, input_external_power_kW):
        excess_power_curtailed_kW = np.where(
            input_external_power_kW > self.cluster_rating_kW,
            input_external_power_kW - self.cluster_rating_kW,
            0,
        )

        # minimum_power_kW = self.turndown_ratio*self.cluster_rating_kW
        below_turndown_power_curtailed_kW = np.where(
            input_external_power_kW < self.min_cluster_power_kW,
            input_external_power_kW,
            0,
        )
        total_curtailed_power_kW = (
            below_turndown_power_curtailed_kW + excess_power_curtailed_kW
        )
    ...
    def create_power_current_curve(self, T_stack):
        ...
        df_size = len(current_range) * len(temp_range)
        powers = np.zeros(df_size)
        currents = np.zeros(df_size)
        temps_C = np.zeros(df_size)
        cell_voltage = np.zeros(df_size)
        current_density = np.zeros(df_size)
        idx = 0
        for i in range(len(current_range)):
            for t in range(len(temp_range)):
                if self.electrolyzer_type == "PEM":
                    ...
                    current_density[idx] = self.calc_current_density(
                        temp_range[t], current_range[i]
                    )
    ...
    def feedstock_usage(self):
        ...
        if self.electrolyzer_type == "ALK":
            self.BOL_design_info.update(
                {
                    "Feedstock Usage: Liters H2O/kg-H2": 11.126,
                    "Feedstock Usage: Grams KOH/kg-H2": KOH_to_H2_usage,
                }
            )
        else:
    ...
    def calc_current_density(self, I_stack, T_stack=None):
        if self.electrolyzer_type == "ALK":
    ...
    def arden_buck(self, T_stack):
        p_h2O_sat_kPa = 0.61121 * np.exp(
            (18.678 - (T_stack / 234.5)) * (T_stack / (257.14 + T_stack))
        )
    ...
    def cell_reversible_overpotential(self, T_stack):
        if self.electrolyzer_type == "ALK":
            ...
            T_K = convert_temperature([T_stack], "C", "K")[0]
            Urev0 = self.cell_Urev0()
            panode_atm = self.anode_pressure * (
                atm / bar
            )  # [atm] total pressure at the anode
            pcathode_atm = self.anode_pressure * (
                atm / bar
            ...
            U_rev = Urev0 + ((self.R * T_K) / (2 * self.F)) * (
                np.log(
                    ((panode_atm - p_H2O_sat_atm) / patmo_atm)
                    * np.sqrt((pcathode_atm - p_H2O_sat_atm) / patmo_atm)
                )
            )
    ...
    def cell_activation_overpotential(self, T_stack, I_stack):
        if self.electrolyzer_type == "ALK":
            ...
            j_eff = self.calc_current_density(I_stack, T_stack)
            ja = j_eff  # [A/cm^2]
            jc = j_eff  # [A/cm^2]
    ...
    def cell_electrode_resistance(self, T_stack=None):
        if self.electrolyzer_type == "ALK":
            tref = 25
    ...
    def cell_degradation(self, V_cell, cluster_status):
        if self.include_degradation_penalty:
            ...
            self.simulation_results.update(
                {"Simulation Final Steady Deg [V/cell]": np.cumsum(V_deg_uptime)[-1]}
            )
            self.simulation_results.update(
                {"Simulation Final On/Off Deg [V/cell]": np.cumsum(V_deg_onoff)[-1]}
            )
            self.simulation_results.update(
                {"Simulation Final Fatigue Deg [V/cell]": V_fatigue[-1]}
            )
        else:
    ...
    def cell_fatigue_degradation(self, V_cell, dt_fatigue_calc_hrs=168):
        ...
        if np.max(V_cell) != np.min(V_cell):
            ...
            rf_track = 0
            t_calc = np.arange(
                0, len(V_cell) + dt_fatigue_calc_hrs, dt_fatigue_calc_hrs
            )
            for i in range(len(t_calc) - 1):
                voltage_signal_temp = V_cell[
                    np.nonzero(V_cell[t_calc[i] : t_calc[i + 1]])
                ]
        ...
        self.simulation_results.update(
            {"Lifetime Fatigue Deg [V/sim]": lifetime_fatigue_deg}
        )
        return V_fatigue_ts
    ...
    def cell_steady_degradation(self, V_cell, cluster_status):
        ...
        self.simulation_results.update(
            {"On-time/sim [hrs]": np.sum(cluster_status) * (self.dt / 3600)}
        )
    ...
    def calc_faradaic_efficiency(self, I_stack, T_stack=None):
        ...
        j = self.calc_current_density(I_stack, T_stack)  # [A/cm^2]
        j *= 1000  # [mA/cm^2]
        # Faradaic Efficiency
        eta_F = f2 * (j**2) / (f1 + j**2)
        return eta_F

    def cell_H2_production_rate(self, I_stack, T_stack=None):
        eta_F = self.calc_faradaic_efficiency(I_stack, T_stack)
    ...
    def stack_reverse_faradays(self, H2_required_per_stack_kg):
        ...
        I_reqd_BOL_noFaradaicLoss = (H2_required_per_stack_kg * 1000 * 2 * self.F) / (
            1 * self.n_cells * self.dt * self.M_H2
        )
        n_f = self.calc_faradaic_efficiency(I_reqd_BOL_noFaradaicLoss)
        I_reqd = (H2_required_per_stack_kg * 1000 * 2 * self.F) / (
            n_f * self.n_cells * self.dt * self.M_H2
        )
