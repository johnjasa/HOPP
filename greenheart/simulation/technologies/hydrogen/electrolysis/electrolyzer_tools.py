...
def plot_IV_curve(electrolyzer: ElectrolyzerCluster,file_desc = "test"):
    ...
    for ii,I_stack in enumerate(current_range):
        ...
        current_density[ii] = electrolyzer.calc_current_density(electrolyzer.T_stack, I_stack)
        V_cell[ii] = electrolyzer.cell_design(electrolyzer.T_stack,I_stack)

        U_rev[ii] = electrolyzer.cell_reversible_overpotential(electrolyzer.T_stack, electrolyzer.pressure_operating)
        ...
        R_a, R_c = electrolyzer.cell_electrode_resistance(electrolyzer.T_stack)
        R_ele_bf, R_ele_b = electrolyzer.cell_electrolyte_resistance(electrolyzer.T_stack, I_stack)  # [Ohms]
        R_membrane = electrolyzer.cell_membrane_resistance(electrolyzer.T_stack)  # [Ohms] 
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(os.path.dirname(__file__), f"{electrolyzer.electrolyzer_type}_IV-Curve-{file_desc}.pdf"),bbox_inches = "tight")
    plt.close()
