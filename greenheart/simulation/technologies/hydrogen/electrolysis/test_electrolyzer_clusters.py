...
from greenheart.simulation.technologies.hydrogen.electrolysis.electrolyzer_tools import get_efficiency_curve
...
class TestElectrolyzerCurves(unittest.TestCase):
    ...
    #     generated_df = get_efficiency_curve(alk, file_desc="July2024")
    ...
    #     generated_df = get_efficiency_curve(pem, file_desc="July2024")

    #     pd.testing.assert_frame_equal(generated_df, truth_df)

    def test_soec_curve(self):
        # Load the saved PEM truth dataframe
        truth_df = pd.read_pickle('pem_curve.pkl')
        
        # Generate the PEM dataframe
        pem = ElectrolyzerCluster(cluster_size_mw=1, plant_life=30, electrolyzer_type="SOEC")
        generated_df = get_efficiency_curve(pem, file_desc="July2024")
