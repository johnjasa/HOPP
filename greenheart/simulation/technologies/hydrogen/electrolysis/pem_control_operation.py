from greenheart.simulation.technologies.hydrogen.electrolysis.PEM_sync_electrolyzer_clusters import PEM_Clusters as pem
...
class PEMSupervisor:
    ...
    def create_clusters(self,num_clusters:int,cluster_size_mw,pem_kwargs):
        
        clusters = []
        if isinstance(cluster_size_mw,(int,float)):
            for i in range(num_clusters):
                clusters.append(pem(cluster_size_mw,self.plant_life,**pem_kwargs))
        else:
            for i,cluster_size in enumerate(cluster_size_mw):
                clusters.append(pem(cluster_size))
        return clusters
...
if __name__ == "__main__":
    ...
    plant_life = 30
    pem_config = {"include_degradation_penalty": True,
    "run_LTA":True,
    "debug_mode": True}
